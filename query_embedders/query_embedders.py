...
class QueryBertEmbedder:
    ...
    def __init__(self, train_queries: list, test_queries: list=None, dev_queries: list=None, embedding_dim: int=768,
                 bert_model_name: str='bert-base-uncased') -> None:
        """
    ...
    def get_queries_embeddings(self) -> None:
        """
...
class QueryTFIDFEmbedder:
    ...
    """
    def __init__(self, corpus: dict, train_queries: list, max_dim: int=3000, test_queries: list=None,
                 dev_queries: list=None) -> None:
        """
...
def save_queries_embeddings_pickle(dataset_name: str, models: list, model_names: list, seed: int) -> None:
    """
    ...
    Args:
        ...
        seed: The seed used for generating the query embeddings.
    """
    ...
    for model, name in zip(models, model_names):
        ...
        with open(f'{dataset_name}/query_embeddings_train_{dataset_name}_{name}_{seed}.pkl', 'wb') as f:
            pkl.dump(model.train_queries_embeddings, f)

        if model.dev_queries is not None:
            with open(f'{dataset_name}/query_embeddings_dev_{dataset_name}_{name}_{seed}.pkl', 'wb') as f:
                pkl.dump(model.dev_queries_embeddings, f)

        if model.test_queries is not None:
            with open(f'{dataset_name}/query_embeddings_test_{dataset_name}_{name}_{seed}.pkl', 'wb') as f:
